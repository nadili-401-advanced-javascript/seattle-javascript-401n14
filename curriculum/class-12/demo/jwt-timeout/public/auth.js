...
const basicAuth = async e => {
  ...
  if (!username || !password)
    ...
    if (authResponse.status === 200) {
      sendMessage(`Successfully logged in and recieved token:\n${token}`);
      console.log(token);
    }
  }
};
...
const useToken = async () => {
  //console.log('separate function, same token', token);
  //send another request to /authenticate

  let authResponse = await fetch('/authenticate', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Accept: 'application/json',
  ...
  if (data.user) sendMessage(`Found User: ${data.user}`);
  else sendMessage(data);
};
